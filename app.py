...
def get_client():
    ...
    if _llm_client: return _llm_type, _llm_client
    ...
    if groq_key:
        ...
        raise EnvironmentError('No API key. Set GROQ_API_KEY or ANTHROPIC_API_KEY.')
...
@app.route('/')
def home(): return send_file('output/dashboard.html')
...
def run():
    ...
    if len(req) > 2000:
        return jsonify({'status': 'error', 'message': 'Request too long (max 2000 chars)'}), 400
...
"@ | Out-File -FilePath C:\s\app.py -Encoding utf8